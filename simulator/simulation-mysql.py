...
class Database:
    ...
    def __init__(self):
        ...
        connection = createDB.cursor()
        connection.execute("CREATE DATABASE IF NOT EXISTS simulation")
        self.db = mysql.connector.connect(
            host="localhost", user="root", password="password", database="simulation")
        connection = self.db.cursor()
        # connection.execute("DROP TABLE simulation")
    ...
    def checkForSimulation(self, name):
        ...
        connection.execute(f"""
                            SELECT EXISTS(SELECT * FROM simulation
                            WHERE name='{name}')""")
    ...
    def getSimulation(self, name):
        ...
        connection.execute(f"SELECT * FROM genome_{name}")
        data["organisms"] = []
        ...
        connection.execute(f"SELECT * FROM generation_{name}")
        data["generations"] = []

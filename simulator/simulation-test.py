...
class Simulation:
    ...
    def start(self):
        ...
        Returns:
        - data (dict): The starting data for the simulation (population size, generation count,
                       organism data and location)
        """
        for i in range(self.populationSize):
            self.createOrganism()

        return self.simulationData.getAllData()
    ...
    def move(self):
        ...
        for i in self.organisms:
            ...
            # if ((i.xCoordinate, i.yCoordinate) in self.coordinates):
            ...
            self.simulationData.updateMovement(self.currentGeneration,
                                               i.id, i.xCoordinate, i.yCoordinate)

    def updateGeneration(self):
        """
        Updates the simulation for a new generation.
        """
        # This is pushed so that the programs knows it is a new generation
        # self.simulationData.updateMovement(self.currentGeneration, -1, -1, -1)

        # The list of coordinates is cleared at the start of every generation
        self.coordinates = {}
        ...
        for i in self.organisms:
            ...
            self.simulationData.updateMovement(
                self.currentGeneration, i.id, newCoord[0], newCoord[1])

        # The organisms move for 100 generations (this can change by changing the grid size)
    ...
    def createOrganism(self):
        ...
        self.simulationData.addOrganism(self.currentGeneration, newOrganism)
...
class SimulationData:
    ...
    def __init__(self, generationCount, populationSize, location):
        """
        Initializes the SimulationData object.

        Args:
            generationCount (int): The number of generations in the simulation.
            populationSize (int): The size of the population in the simulation.
        ...
        """
        self.__data = {}
        self.__data["generationCount"] = generationCount
        self.__data["populationSize"] = populationSize
        # A list of (id, (xCoordinate, yCoordinate))
        # A list of (id, genes, parents)
        self.__data["organisms"] = [[] for i in range(generationCount+2)]
        self.__data["generations"] = [[] for i in range(generationCount+2)]
    ...
    def getData(self, generation):
        ...
        """
        generationData = {}
        generationData["organisms"] = self.__data["organisms"][generation]
        generationData["movement"] = self.__data["generations"][generation]
    ...
    def uploadData(self, data):
        ...
        try:
            ...
            for i in range(generationCount):
                # Checks if all the organisms in each generation is in the data dictionary
                for organism in data["organismsIngeneration"][i]:
    ...
    def addOrganism(self, generation, organism):
        """
        ...
        """
        self.__data["organisms"][generation].append(
            [organism.id, organism.genome.genes, organism.getParents()])

    def updateMovement(self, generation, id, xCoordinate, yCoordinate):
        """
        ...
        """
        self.__data["generations"][generation].append(
...
class Database:
    ...
    def insertSimulation(self, name, password, public, data):
        ...
        connection.execute(
            f"INSERT INTO simulation VALUES ('{name}', '{hashedPassword}', {public}, {generationCount}, {size}, {location_x}, {location_y})")
        ...
        # Creates a table for the generation data
        connection.execute(f"""CREATE TABLE IF NOT EXISTS generation_{name}(
                            id integer PRIMARY KEY,
                            data text)
                           """)
        ...
        for i in range(data["generationCount"]):
            ...
            if (data["generations"][i] == []):
                continue
            # Converts each generation into a string in the format: id, xCoordinate, yCoordinate;id, xCordinate, yCoordinate;...
            # So each move is separated by a semicolon and each value is separated by a comma
            generationData = ""
            for generation in data["generations"][i]:
                generationData += f"{generation[0]},{generation[1][0]},{generation[1][1]};"

            connection.execute(
                f"INSERT INTO generation_{name} VALUES({i}, '{generationData}')")
            generationData = ""
        connection.commit()
        connection.close()
    ...
    def removeSimulation(self, name):
        ...
        connection.execute(f"DROP TABLE generation_{name}")
        connection.commit()
        connection.close()
    ...
    def getSimulation(self, name):
        ...
        # value is in the format (name, password, public, generationCount, populationSize)
        for value in row:
            data["generationCount"] = value[3]
        ...
        rows = connection.execute(f"SELECT * FROM generation_{name}")
        data["generations"] = []
        ...
        for value in rows:
            ...
            # generationData needs to be in the format [[id, [xCoordinate, yCoordinate]], ...]
            generationData = []
            ...
            for move in movements:
                ...
                splitted = move.split(',')
                generationData.append(
                    [int(splitted[0]), [int(splitted[1]), int(splitted[2])]])
            data["generations"].append(generationData)
        return data

...
def start():
    ...
    if (request.method == "POST"):
        ...
        try:
            ...
            size = -1
        try:
            numberOfgenerations = int(request.form["generation"])
        except ValueError:
        ...
        if numberOfgenerations < 1 or numberOfgenerations > 100:
            numberOfgenerations = -1
            error += "Number of generations has to be an integer between 1 and 100. "
        ...
        if (error != ""):
            return render_template("start.html", data={"size": size, "generationCount": numberOfgenerations, "location": locationType, "mutationRate": mutationRate, "error": error})
        ...
        simulationData = simulation.SimulationData(
            int(numberOfgenerations), int(size), locationCoord)
...
def uploadSimulation():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            simulationData = simulation.SimulationData(
                int(data["generationCount"]), int(data["populationSize"]), (data["location"][0], data["location"][1]))
        except KeyError:
...
def getNextgeneration(id):
    """
    Retrieves the next generation data for a given simulation ID.
    The request method can only be GET.
    ...
    Returns:
        str: The data for the next generation.
    """
    ...
    # This is so that the data from the previous generations is not included in the response
    simulationData.clearMovement()
    simulationData.clearOrganisms()
    # Perform the next generation
    newSimulation.updateGeneration()
    # Retrieve the data for the next generation
...
def viewSummary(id):
    ...
    try:
        ...
        # A list of genomes for each organism in the generation
        for i in range(data["generationCount"]):
            variance = []
            for j in data["organismsInGeneration"][i]:
                variance.append(data["organisms"][j][1])
            generationNumber.append(i+1)
            # Calculate the variance for all the genomes in the generation
        ...
        # Calculate fitness score for each generation
        for i in range(data["generationCount"]):
            # data[generations][i] is in the format [[organismID, [x, y]], [organismID, [x, y]], ..., ...]
            generationData = data["generations"][i]
            # Calculates the total number of moves in the generation
            totalSteps = len(generationData)
            # The score sum is the total distance from the fitness location to all the organisms in the generation
            scoreSum = 0
            # Calculate the distance from the fitness location to each organism in the generation
            for j in range(data["populationSize"]):
                # generationData[j] is in the format [organismID, [x, y]]
                coordinates = generationData[totalSteps-j-1][1]
    ...
    plt.ylim(0, 1)
    if (len(generationNumber) == 1):
        # If there is only one generation, plot a single point
        plt.plot(generationNumber, varianceData, "o")
    else:
        # Plot the graph with generationNumber as the x-axis and varianceData as the y-axis
        plt.plot(generationNumber, varianceData)
    # Set the x-axis label to "Generation"
    ...
    plt.clf()
    if (len(generationNumber) == 1):
        # Plot a single point if there is only one generation
        plt.plot(generationNumber, fitnessData, "o")
    else:
        # Plot the graph with generationNumber as the x-axis and fitnessData as the y-axis
        plt.plot(generationNumber, fitnessData)
    # Label the x-axis as "Generation"
    plt.xlabel("Generation")
